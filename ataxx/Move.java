...
class Move {
     ...
     */
    boolean isExtend() {
        int coldiff = Math.abs(_col1 - _col0);
        int rowdiff = Math.abs(_row1 - _row0);
    ...
    boolean isJump() {
        int coldiff = Math.abs(_col1 - _col0);
        int rowdiff = Math.abs(_row1 - _row0);
        if (!isExtend() && coldiff <= 2 && rowdiff <= 2) {
            return true;
        }
        return false;
    }
    ...
    private static final Move[][][][] ALL_MOVES =
            new Move[EXTENDED_SIDE][EXTENDED_SIDE][EXTENDED_SIDE]
                    [EXTENDED_SIDE];
